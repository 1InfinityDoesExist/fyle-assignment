...
package com.fyle.assignment.util;
...
public abstract class AbstractEncryptDecryptConverter<X> implements AttributeConverter<X, String> {
	...
	private static final String SECRET_ENCRYPTION_KEY = "RXGHTDSQOLKJTRWX";
	...
	private X decryptData(Cipher cipher, String dbData) throws IllegalBlockSizeException, BadPaddingException {
		byte[] bytesToEncrypt = ((String) convertStringToEntityAttribute(dbData)).getBytes();
	...
	public static String encryptUserTable(String strToEncrypt, String secret) {

		try {
			CipherMaker c = new CipherMaker();
			Cipher cipher = c.configureAndGetInstance(Cipher.ENCRYPT_MODE, SECRET_ENCRYPTION_KEY);
